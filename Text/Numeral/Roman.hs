...
data NumeralConfig s n = NC
    { -- |Symbol to represent the value 0. The Romans did not have a
      ...
      -- throw an error.
      ncZero ∷ Maybe s
      -- |Symbol to represent the value 1.
    , ncOne  ∷ s
...
convertTo nc n
    ...
    where go _ [] = errMsg "out of symbols"
          go i tab@(~(sym, val) : ts)
              | i ≤ 0     = return (∅)
              | i ≥ val   = liftM2 (⊕) (return sym) $ go (i - val) tab
              | otherwise = go i ts
